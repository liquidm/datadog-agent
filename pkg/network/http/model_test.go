...
func TestMaximumLengthPath(t *testing.T) {
	tx := httpTX{
		request_fragment: requestFragment(
			[]byte("GET /aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabc"),
		),
	}
	b := make([]byte, HTTPBufferSize)
	path, fullPath := tx.Path(b)
	assert.Equal(t, "/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaab", string(path))
